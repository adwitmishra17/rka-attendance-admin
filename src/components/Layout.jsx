...
const NAV = [
  ...
  {
    to: '/reports/monthly', label: 'Monthly Reports', icon:
      <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
  ...
  {
    to: '/reports/advances', label: 'Salary Advances', icon:
      <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
        <rect x="2" y="6" width="20" height="12" rx="2" /><circle cx="12" cy="12" r="2.5" />
      </svg>
  },
  {
    to: '/payroll', label: 'Payroll', superAdminOnly: true, icon:
      <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
...
export default function Layout() {
  const { user, adminRole, isSuperAdmin, isReceptionist, currentBranch } = useAuth()
  ...
  // Used in sidebar profile section — small branch indicator visible to everyone.
  // Super admin sees "All Branches" / "Main Campus" / "City Branch".
  // Branch admin/receptionist sees their locked branch.
  const sidebarBranchLabel = branchLabel(currentBranch)
      ...
      <div style={{ borderTop: '1px solid var(--gray-100)', padding: '12px 16px' }}>
        ...
        <div style={{
          ...
          <a href="https://skolix.app" target="_blank" rel="noopener noreferrer" style={{ color: 'inherit', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
            Powered by
            <svg width="12" height="12" viewBox="0 0 64 64" aria-hidden="true" style={{ display: 'block', flexShrink: 0 }}>
